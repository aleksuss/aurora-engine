...
pub fn submit_modexp<I: IO + Copy, E: Env, P: PromiseHandler, M: ModExpAlgorithm + 'static>(
    io: I,
    env: &E,
    args: &SubmitArgs,
    state: EngineState,
    ...
    let transaction = deserialize_transaction(&args.tx_data)?;
    #[cfg(not(feature = "contract"))]
    let transaction = deserialize_transaction(&args.tx_data, env.block_height())?;
    // Retrieve the signer of the transaction:
    let sender = transaction.address;
    sdk::log!("signer_address {:?}", sender);

    let engine: Engine<I, E, M> =
        Engine::new_with_state(state, sender, current_account_id, io, env);

    submit_transaction(io, engine, transaction, args, relayer_address, handler)
}
...
pub fn submit<I: IO + Copy, E: Env, P: PromiseHandler>(
    ...
    let sender = transaction.address;
    sdk::log!("signer_address {:?}", sender);
    let engine: Engine<I, E> = Engine::new_with_state(state, sender, current_account_id, io, env);

    submit_transaction(io, engine, transaction, args, relayer_address, handler)
}

fn submit_transaction<I: IO + Copy, E: Env, P: PromiseHandler, M: ModExpAlgorithm + 'static>(
    mut io: I,
    mut engine: Engine<I, E, M>,
    transaction: NormalizedEthTransaction,
    args: &SubmitArgs,
    relayer_address: Address,
    handler: &mut P,
    ...
    if let Some(chain_id) = transaction.chain_id {
        if U256::from(chain_id) != engine.chain_id() {
            return Err(EngineErrorKind::InvalidChainId.into());
        }
    }

    let sender = engine.origin;
...
#[cfg(feature = "contract")]
fn deserialize_transaction(tx_data: &[u8]) -> EngineResult<NormalizedEthTransaction> {
    let transaction = NormalizedEthTransaction::try_from(
        EthTransactionKind::try_from(tx_data).map_err(EngineErrorKind::FailedTransactionParse)?,
    )
...
fn deserialize_transaction(
    tx_data: &[u8],
    block_height: u64,
) -> EngineResult<NormalizedEthTransaction> {
    ...
    let transaction: NormalizedEthTransaction = {
        ...
        let tx: EthTransactionKind = adapter
            .try_parse_bytes(tx_data, block_height)
